...
无 `net.fetch` / `fs.*` / `clipboard.*` / `agent.*`。宿主会净化 CSS：禁止 `@import`，`url()` 只能是 `data:` 或声明在 `contributes.themes[].assets` 里的资源。

PI-Desktop **0.15.6** 的 host-core 在 `installFromPath` 时要求 theme assets 为**本机绝对路径**。仓库里保持可移植的相对路径（`fonts/*.woff2`）；用开发加载或市场安装时由宿主解析。若 Settings 列表没有「Craftdesk 方野」，检查 `~/.pi-desktop/logs/app/plugin.log` 是否出现 `plugin.themes.skipped` / `INVALID_CSS`。
